...
function lmer for mixed effect model
lme function for fixed effect model
...
##spp_rcbd_mix_mod <- lmer(yield_kgha ~ krate_kgha*nrate_kgha*prate_kgha + (1|rep/krate_kgha/nrate_kgha)) its a nesting
...
- Normality  (qq and desnity plot as well)
...
```
-nrate_kgha:krate_kgha:SE =330, df =22.9
-nrate_kgha: SE =209, df =8.19
-krate_kgha: SE =232, df = 8.28
...
```{r k rate means}
emmeans(spp_rcbd_mix_mod, ~krate_kgha )
```
...
```{r n rate means }
emmeans(spp_rcbd_mix_mod, ~nrate_kgha )
```
It is 209.  

Points to notice:  
  - The **SE of the WP (krate) and SP (nrate) treatment factors is different**! On the RCBD case we ran last exercise, they were the same because we only had one experimental size unit.  
